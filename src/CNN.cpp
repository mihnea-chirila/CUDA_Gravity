...
CNN::CNN(int in_layers, std::vector<int> in_nodes, int in_batch_size){
    ...
    for(int i=0; i<layers; i++){
        ...
        cudaMalloc((void**)&d_W, sizeof(float)*nodes_per_layer[i]*nodes_per_layer[i+1]);
        cudaMalloc((void**)&d_b, sizeof(float)*nodes_per_layer[i+1]);
        cudaMalloc((void**)&d_dW, sizeof(float)*nodes_per_layer[i]*nodes_per_layer[i+1]);
        cudaMalloc((void**)&d_db, sizeof(float)*nodes_per_layer[i+1]);
    }
    ...
    for(int i=0; i<layers-1; i++){
        ...
        for(int j=0; j<nodes_per_layer[i]; j++){
            for(int k=0; k<nodes_per_layer[i+1]){
...
void CNN::set_batch(float *b_data, int *label, int in_batch_size, int in_dim){
    ...
    cudaMemcpy(d_label, label, sizeof(int)*batch_size, cudaMemcpyHostTodDevice);
...
void CNN::feed_forward_stream(cudaStream_t* stream){
    ...
    for(int i=1; i<layers; i++){
        ...
        //sigmoid is not performed on the last layer
        if(i < layers)
            sigmoid<<<batch_size*nodes_per_layer[i]/512 + 1, 512, *stream>>>(d_A[i], batch_size*nodes_per_layer[i]);
    }
    softmax<<<batch_size/512 + 1, 512, *stream>>>(d_A[layers-1], d_p, batch_size, nodes_per_layer[layers-1]);
...
void CNN::back_prop(){
    ...
    for(int i=layers-3; i>=0; i--){
        ...
        calc_delta<<<batch_size*nodes_per_layer[l+1]/512, 512>>>(d_delta[i], d_A[i+1],
