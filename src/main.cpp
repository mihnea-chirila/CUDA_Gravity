...
static void report_time(std::chrono::time_point<std::chrono::steady_clock> start,
                        ...
                        int iters) {
  auto usecs = std::chrono::duration_cast<std::chrono::duration<float, std::chrono::microseconds::period>>(end - start);
  std::cout << usecs.count() / iters << std::endl;
}
...
int main(int argc, char** argv){
  ...
  std::cout << "Reading data and making of Neural Netowrk objects for training and validation" << std::endl;
  std::ifstream in_file;  
  int train_batch_size = 10;//000;
  CNN train_nn(num_nodes.size(), num_nodes, train_batch_size);
  ...
  for(int i = 0; i < train_data_size; i++)
  {
    ...
    // std::cout << train_label[i] << " ";
  }
  in_file.close();

  // validation data set
  int validation_data_size = 10000;
  CNN validation_nn(num_nodes.size(), num_nodes, validation_data_size);
  ...
  for(int n = 0; n < num_steps; n++)
  {
    ...
    if(n%(num_steps/10)==0){
      ...
      std::cout << "training accuracy: " <<  train_nn.compute_accuracy() << ", ";
      std::cout << "validation accuracy: " <<  validation_nn.compute_accuracy() << std::endl;
    }
  }

  std::cout << std::endl << "////////////////////////////////////////////////////////////" << std::endl;
  std::cout << "Evaluation of the model on the testing data:" << std::endl;
  std::cout << "Reading data and making Neural Netowrk objects for testing" << std::endl;  
  // read test image
  ...
  CNN test_nn(num_nodes.size(), num_nodes, test_data_size);
  ...
  uint8_t streams = 32;
  launchStreams(streams, &test_nn);
  auto end = std::chrono::steady_clock::now();
  std::cout << "Cost: " << std::fixed << std::setprecision(3) << test_nn.compute_cost() << ", ";
  std::cout << "test accuracy: " <<  test_nn.compute_accuracy() << std::endl;
  std::cout << "Testing time: ";
  report_time(start, end, test_data_size);
  return 0;
}
