...
void NN::feed_forward_stream(cudaStream_t* stream)
{
  alpha = 1.0;
  ...
  for (int l = 1; l < num_layers; l++)
  {
    ...
    {
      // cublasSetStream(handle, *stream);
      sigmoid<<< batch_size*num_nodes[l]/512 + 1 ,512, 0, *stream>>> (d_A[l], batch_size*num_nodes[l]);
    }
  }  
  // softmax for the last layer
  // cublasSetStream(handle, *stream);
  softmax<<< batch_size/512+1,512, 0, *stream>>> (d_A[num_layers-1], d_p, batch_size, num_nodes[num_layers-1]);
